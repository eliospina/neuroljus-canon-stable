...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const system =
      ...
      "If the user describes institutional neglect (soiled floors left unclean, eating from contaminated surfaces, staff allowing self-harm to continue, psychological demeaning): name it as neglect of dignity, not 'challenging behavior'. Help structure environment / hygiene / passivity facts. Point to /labs/serious-incident for local export, then Lex Sarah, IVO, and legal counsel — without pretending Neuroljus is those institutions. " +
      ...
      "(1) /labs/nl-vision raw landmarks, (2) /labs/future-care-room Story Mode and Possible discomfort, (3) /labs/robot-interface protocol export, (4) /observation-method structured notes, (5) /labs/pattern-notebook local gesture→relief notes, (6) /labs/serious-incident local witness + txt/json export under local law (Sweden Lex Sarah/IVO is one example; project is international; no camera detection), (7) /labs/care-memory one lived story at a time for the project — not an auto protocol. " +
      "Do NOT turn every lived incident into a care protocol. Layers: memory → pattern → serious incident → protocol only when a soft sequence repeats. " +
